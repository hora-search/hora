//! SIMD-optimized dot product and distance implementations (x86_64, aarch64, wasm32, scalar fallback).

/// Trait for SIMD-optimized vector operations (dot product, Manhattan, Euclidean).
/// Implemented for `f32` and `f64` with optional arch-specific code paths.
pub trait SIMDOptimized<T = Self> {
    ...
    impl SIMDOptimized for f32 {
        fn dot_product(a: &[f32], b: &[f32]) -> Result<f32, &'static str> {
            ...
            unsafe {
                ...
                let simd_sum = f32x4_extract_lane::<0>(sum)
